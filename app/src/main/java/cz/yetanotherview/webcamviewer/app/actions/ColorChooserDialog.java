...
public class ColorChooserDialog extends DialogFragment implements View.OnClickListener {
    ...
    private int[] mColors;
    private boolean fromTextButton;

    @Override
    public void onClick(View v) {
        if (v.getTag() != null) {
            Integer index = (Integer) v.getTag();
            mCallback.onColorSelection(index, mColors[index], shiftColor(mColors[index]), fromTextButton);
            dismiss();
        }
    }

    public interface Callback {
        void onColorSelection(int index, int color, int darker, boolean from);
    }
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        MaterialDialog dialog = new MaterialDialog.Builder(getActivity())
                ...
                .autoDismiss(false)
                .customView(R.layout.color_chooser_dialog, false)
                .build();
    ...
    public void show(Activity context, int preselect, boolean fromTextButton) {
        Bundle args = new Bundle();
        args.putInt("preselect", preselect);
        args.putBoolean("fromTextButton", fromTextButton);
        setArguments(args);
